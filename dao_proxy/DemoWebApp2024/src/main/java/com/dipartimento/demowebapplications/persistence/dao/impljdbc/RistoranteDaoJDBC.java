...
public class RistoranteDaoJDBC implements RistoranteDao {
    ...
    @Override
    public List<Ristorante> findRistorantiByPiattoNome(String nomePiatto) {

        List<Ristorante> ristoranti = new ArrayList<>();
        String query = "SELECT r.id, r.nome FROM Ristorante r " +
                "JOIN ristorante_piatto rp ON r.id = rp.ristorante_id " +
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                int ristoranteId = resultSet.getInt("id");
